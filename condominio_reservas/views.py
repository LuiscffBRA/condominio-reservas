...
def buscar_ocupacao(request, area_id):
    try:
        reservas = Reserva.objects.filter(
            areaComum_id=area_id, status="Aprovado"
        ...
        for r in reservas:
            ...
            if r.horarioFim <= r.horarioInicio:
                fim = datetime.datetime.combine(
                    r.dataReserva + datetime.timedelta(days=1), r.horarioFim
                )
            else:
...
def home_sindico(request):
    total_moradores = Morador.objects.filter(statusConta="Aprovado").count()
    total_pendentes = Reserva.objects.filter(Q(pago=False, status="Aprovado") | Q(status="Cancelado_Multa")).count()
    
    hoje = datetime.date.today()
    total_hoje = Reserva.objects.filter(dataReserva=hoje, status="Aprovado").count()

    trinta_dias_atras = hoje - datetime.timedelta(days=30)
    reservas_30_dias = Reserva.objects.filter(dataReserva__gte=trinta_dias_atras, status="Aprovado")
    total_reservas_30 = reservas_30_dias.count()

    grafico_labels = []
    grafico_data = []

    if total_reservas_30 > 0:
        areas_stats = reservas_30_dias.values('areaComum__nome').annotate(total=Count('id')).order_by('-total')
        for stat in areas_stats:
            grafico_labels.append(stat['areaComum__nome'])
            percentual = round((stat['total'] / total_reservas_30) * 100)
            grafico_data.append(percentual)
    else:
        grafico_labels = ['Nenhuma reserva']
        grafico_data = [100]
...
def listar_todos_moradores(request):
    moradores = (
        Morador.objects.filter(statusConta__in=["Aprovado", "Desabilitado"])
...
def cancelar_reserva(request, id):
    if request.method == "POST":
        try:
            is_admin = request.user.is_superuser or hasattr(request.user, "sindico") or hasattr(request.user, "administrador")
            
            if is_admin:
                reserva = Reserva.objects.get(id=id)
            else:
                reserva = Reserva.objects.get(id=id, morador=request.user.morador)
            ...
            if reserva.pago and dias_restantes < reserva.areaComum.prazoCancelamentoDias:
                ...
                messages.warning(
                    request,
                    f"Reserva da área {reserva.areaComum.nome} cancelada. Uma multa por cancelamento tardio foi gerada."
                )
            else:
                reserva.delete()
            ...
            messages.error(request, "Reserva não encontrada ou você não tem permissão.")
            
    if request.user.is_superuser or hasattr(request.user, "sindico") or hasattr(request.user, "administrador"):
...
def editar_reserva(request, id):
    if request.method == "POST":
        try:
            ...
            if not is_admin and reserva.pago and dias_restantes < reserva.areaComum.prazoCancelamentoDias:
                return JsonResponse({
                    "sucesso": False, 
                    "erros": ["Esta reserva está paga e fora do prazo permitido para edição."]
                })

            form = ReservaForm(request.POST, instance=reserva, area=reserva.areaComum)
            ...
            return JsonResponse({"sucesso": False, "erros": ["Reserva não encontrada ou sem permissão."]})
...
def listar_reservas(request):
    ...
    return render(
        ...
        {"reservas": reservas, "areas_comuns": areas_comuns},
    )
...
def alternar_pagamento_reserva(request, id):
    if request.method == "POST":
        try:
            ...
            reserva.save()
            if reserva.pago:
                messages.success(request, f"Reserva de {reserva.morador.first_name} marcada como PAGA.")
            else:
                messages.warning(request, f"Reserva de {reserva.morador.first_name} alterada para NAO PAGA.")
        except Reserva.DoesNotExist:
...
def ver_ocupacao_atual(request):
    hoje = datetime.date.today()
    reservas_hoje = Reserva.objects.filter(dataReserva=hoje, status="Aprovado").order_by("horarioInicio")
...
@login_required(login_url="login")
def relatorio_oficial(request):
    if not (request.user.is_superuser or hasattr(request.user, "sindico") or hasattr(request.user, "administrador")):
        return redirect("home_morador")
        
    hoje = datetime.date.today()
    
    total_moradores = Morador.objects.filter(statusConta="Aprovado").count()
    total_reservas = Reserva.objects.filter(status="Aprovado").count()
    
    reservas_ativas = Reserva.objects.filter(status="Aprovado").order_by("-dataReserva")
    reservas_multadas = Reserva.objects.filter(Q(pago=False, status="Aprovado") | Q(status="Cancelado_Multa")).order_by("-dataReserva")
    areas = AreaComum.objects.all().order_by("nome")

    return render(request, "relatorio_oficial.html", {
        "hoje": hoje.strftime("%d/%m/%Y"),
        "sindico_nome": request.user.first_name,
        "total_moradores": total_moradores,
        "total_reservas": total_reservas,
        "reservas_ativas": reservas_ativas,
        "reservas_multadas": reservas_multadas,
        "areas": areas,
    })